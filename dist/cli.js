...
exports.cli = exports.ChiCTOCLI = void 0;
...
class ChiCTOCLI {
    constructor() {
        this.orchestrator = new orchestrator_1.ModeBAOrchestrator();
    ...
    async handleSuggest(args) {
        ...
        const activeTasksPath = path_1.default.join(projectPath, 'working/active-tasks.md');
        if (!fs_1.default.existsSync(activeTasksPath)) {
            return `Error: No active-tasks.md found in ${projectPath}`;
        }
        try {
            // Read and parse features
            const markdown = fs_1.default.readFileSync(activeTasksPath, 'utf-8');
    ...
    async handleModeB(args) {
        ...
        try {
            ...
            const sessionOrchestrator = new session_management_1.SessionOrchestrator({
                projectPath,
                tokenBudget,
                tokenThreshold: 70,
                features: []
    ...
    async handleStatus(args) {
        const projectPath = args.projectPath || process.cwd();
        const handoverPath = path_1.default.join(projectPath, 'handover.md');
        if (!fs_1.default.existsSync(handoverPath)) {
            return `No previous session found for ${projectPath}`;
        }
        try {
            const handoverContent = fs_1.default.readFileSync(handoverPath, 'utf-8');
...
exports.ChiCTOCLI = ChiCTOCLI;
// Export for use in index.ts and slash commands
exports.cli = new ChiCTOCLI();
