...
export interface ScoredFeature extends Feature {
  baseScore: number;    // U+I+C+M before multiplier
  finalScore: number;   // After multiplier (-1 if flagged)
  tier: 'IMMEDIATE' | 'SOON' | 'LATER' | 'SKIP' | 'FLAGGED';
  reason: string;       // Why this tier/score
}
...
export class PriorityScorer {
   ...
   */
  applyMultiplier(feature: Feature, baseScore: number): { finalScore: number; reason: string } {
  ...
  getTier(score: number): 'IMMEDIATE' | 'SOON' | 'LATER' | 'SKIP' | 'FLAGGED' {
    ...
    if (score >= 20) return 'SOON';
    if (score >= 10) return 'LATER';
    return 'SKIP';
  }
  ...
  scoreBatch(features: Feature[]): ScoredFeature[] {
    ...
    return scored.sort((a, b) => {
      ...
      if (a.tier === 'FLAGGED' && b.tier !== 'FLAGGED') return -1;
      if (a.tier !== 'FLAGGED' && b.tier === 'FLAGGED') return 1;
